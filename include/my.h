/*
** EPITECH PROJECT, 2023
** B-PSU-200-PAR-2-1-minishell1-suleman.maqsood
** File description:
** my.h
*/

#ifndef MY_H_
    #define MY_H_

    #define ERROR 84
    typedef struct env_s {
        char **bin;
        int nbw;
        char *old_pwd;
        char **fon;
        char **env;
        int nbl_denv;
        int one;
        int status;
    }env_t;

    typedef struct fon_s {
        char **fon;
        struct fon_s *next;
        struct fon_s *prec;
    }fon_t;


char **my_str_to_word_array(char *str);
int my_strlen(char *str);
int my_execute(char **env, env_t *en);
char **stock_mypath(char **env, env_t *en);
int create_cd(char *fon, char **env, env_t *en);
int my_putstr(char const *str);
int check_fonction(char **env, env_t *en);
char **stock_env(char **env, env_t *en);
int print_env(env_t *en);
int set_env(env_t *en);
void free_fon(env_t *en);
int create_new_env(env_t *en);
int free_ele_env(env_t *en);
int un_set_env(env_t *en);
int delete_ele_env(int l, env_t *en);
int cd_to_home(env_t *en);
int find_pwd(char **env);
int find_home(char **env);
int cd_minus(env_t *en);
int find_old_pwd(char **env);
int put_old_pwd(env_t *en);
int set_env_void(env_t *en);
int create_new_env_void(env_t *en);
int create_new_env_for_cd(env_t *en);
int help_check_setenv(char **env, env_t *en);
void write_stdlineno(void);
void minishell(char *line, char **env, env_t *en);
fon_t *add_pile(fon_t *node);
char **my_array_to_word_operater(char **str, fon_t *fon);
void *free_pile(fon_t *a);
int count_letter(char *str, int i);
int my_special_strlen(char *str, int i);
int count_word(char *str);
int operater_is_here(char **env, env_t *en, fon_t *fon);
int exit_fon(char **env, env_t *en);
int special_direction(char **fon, char **env, env_t *en, fon_t *tmp);
int create_void_file(char *name);
int check_fonction_right(char **fon, char **env, env_t *en, fon_t *tmp);
int check_cd(char **env, env_t *en);
int check_unsetenv(env_t *en, int *exit);
int second_time(env_t *en);
int special_for_right_my_execute(char **env, env_t *en, fon_t *tmp);
int special_right_help_check_setenv(char **env, env_t *en, char *name);
int special_for_right_env_check(env_t *en, char *name);
int special_for_right_print_env(env_t *en, char *name);
int check_cd_right(char **env, env_t *en, fon_t *tmp);
int already_exist_void_file(char *name);
int special_double_right_help_check_setenv(char **env, env_t *en, char *name);
int special_for_double_right_env_check(env_t *en, char *name);
int special_for_double_right_print_env(env_t *en, char *name);
int check_cd_right_right(char **env, env_t *en, fon_t *tmp);
int double_right(char **fon, char **env, env_t *en, fon_t *tmp);
int special_for_double_right_my_execute(char **env, env_t *en, fon_t *tmp);
int check_fonction_op(char **env, env_t *en);
int check_fonction_pipe(char **fon, char **env, env_t *en, fon_t *tmp);

#endif
